...
private:
    ...
    {
        bool startParsing;
        EntityId currentEntityId = InvalidEntityId;
        char componentName[32];

        inline void Init()
        {
            startParsing = false;
            currentEntityId = InvalidEntityId;
            componentName[0] = '\0';
        }
        ...
        inline void Process(Action action, const char *string, size_t stringLength)
        {
            switch (action)
            {
    ...
    static inline Parser *parser = new Parser();
...
#define HIRQ 0x0008UL
    ...
    static inline iso9660_filelist_entry_t FindFile(const char *fileName)
    {
        iso9660_filelist_entry_t file = {};
        iso9660_filelist_t *fileList = SatAlloc::Malloc<iso9660_filelist_t>();
        fileList->entries = SatAlloc::Malloc<iso9660_filelist_entry_t>(ISO9660_FILELIST_ENTRIES_COUNT);
        ...
        SatAlloc::Free(fileList->entries);
        SatAlloc::Free(fileList);
        return file;
    }
    ...
    static inline int flag_wait(uint16_t flag)
    {
    ...
    static inline int ProcessData(uint16_t offset, uint16_t buffer_number, uint32_t bytes_to_read, uint32_t sectors_to_read)
    {
