...
private:
    ...
    static inline int32_t ConvertToBase2(int32_t srcBase10)
    {
...
public:
    ...
    inline Fxp(const char *str) { Parse(str); }
    ...
    static inline void AsyncDivSet(const Fxp &dividend, const Fxp &divisor)
    {
        cpu_divu_fix16_set(dividend.value, divisor.value);
    }
    static inline Fxp AsyncDivGet() { return (int32_t)cpu_divu_quotient_get(); }

    /***********Member Functions************/

    inline Fxp Sqrt() const { return fix16_sqrt(value); }
    inline Fxp Square() const { return *this * *this; }
    inline Fxp Abs() const { return fix16_abs(value); }
    inline Fxp Tan() const { return fix16_tan(value); }
    inline const int32_t &Value() { return value; }

    /**************Operators****************/

    inline Fxp &operator+=(const Fxp &fxp)
    {
        value += fxp.value;
        return *this;
    }
    ...
    inline Fxp operator/(const Fxp &fxp) const
    {
        cpu_divu_fix16_set(value, fxp.value);
        return (int32_t)cpu_divu_quotient_get();
    }

    inline Fxp operator*(const Fxp &fxp) const { return fix16_mul(value, fxp.value); }
