...
void RBTree_Insert(RBTree tree, int key, void *data)
{
    ...
    {
        AssignParent(tree.root, newNode);
...
{
    RBNode *search = Search(tree.root, key);

    if (search == NULL || search == tree.root)
        return NULL;
    else
        return search->data;
}
...
{
    RBNode *node = Search(tree.root, key);
    if (node == NULL)
    {
        return false;
    }
    ...
    else
    {
        ...
        {
            Transplant(&tree.root, minimumNode, minimumNode->right);
            minimumNode->right = node->right;
            minimumNode->right->parent = minimumNode;
        }
        Transplant(&tree.root, node, minimumNode);
