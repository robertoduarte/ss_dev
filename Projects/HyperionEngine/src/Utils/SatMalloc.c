...
int GetBlockPosition(void *address, Block list[])
{
    return (address - (void *)list) / sizeof(Block);
}

int MaskAsFree(int value)
{
    return -abs(value);
}

int ClearMask(int value)
{
    return abs(value);
}

bool CheckIfFree(int value)
{
    return value < 0;
}
...
void MakeNewBlock(Block list[], int c, int blocks, bool applyMask)
{
...
void DisconnectFromFreeList(Block list[], int c)
{
...
void AssimilateUp(Block list[], int c)
{
...
int AssimilateDown(Block list[], int c, bool applyMask)
{
...
void FreeFromHeap(Block list[], void *ptr)
{
