...
void _ActDefaultMessageHandler(ACTOR* current, MESSAGE_FLAGS message, long data)
{
    ...
    case ASYNC_MESS_RS_RUN:
    case ASYNC_MESS_RS_WAIT:
    case ASYNC_MESS_RS_SLEEP:
    case ASYNC_MESS_RS_SUSPEND:
...
void ActSendMessage(ACTOR* receive, MESSAGE_FLAGS mess, long data)
{
    ...
    if (mess & ASYNC_MESS_BROADCAST)
    {
        assert(receive == NULL); // required for broadcast

        // resend to each

        TreeIterateValue(&actor_tree, _ActBroadcastCallback, mess & ~ASYNC_MESS_BROADCAST, (long*) data);
        return;
    }
    assert(receive != NULL);
    ...
    case AFLAG_RS_SLEEP:
        if (mess > ASYNC_MESS_PRIORITY_2)
            block = TRUE;
        break;

    case AFLAG_RS_SUSPEND:
        if (mess > ASYNC_MESS_PRIORITY_1)
            block = TRUE;
        break;
...
void ActKillActorTree(void)
{
    ...
    if (actor_tree_extant)
        ActSendMessage(NULL, ASYNC_MESS_BROADCAST | ASYNC_MESS_DESTROY, 0);
...
// Once the actor is created, an ASYNC_MESS_CREATE message is sent to that
                      ...
                      int scratch)
{
    ...
    ActSendMessage(new_actor, ASYNC_MESS_CREATE, (long) parent);
...
void _ActAutoUpdate(ACTOR* curr)
{
    ...
    {
        ActSendMessage(curr, ASYNC_MESS_DESTROY, 0);
    }
